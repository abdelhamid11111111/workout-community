...
const ChallengeCard = ({ userChallenge, onDelete }: ChallengeCardProps) => {
  const workoutCount = userChallenge.workoutCount;
  ...
  // Still fetched client-side for now, since streak isn't in the backend payload yet
  useEffect(() => {
    const fetchStreak = async () => {
      try {
        const res = await fetch(`/api/workout/${userChallenge.challenge.id}`);
  ...
  const progressPercent =
    Number(userChallenge.challenge.days) > 0
      ? Math.round((workoutCount / Number(userChallenge.challenge.days)) * 100)
      : 0;
