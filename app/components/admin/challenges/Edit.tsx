...
export default function EditChallengesPage({ categories, levels }: Props) {
  ...
  useEffect(() => {
    const fetchChallenge = async () => {
      try {
        ...
        // fill goals - fall back to at least 3 empty rows
  ...
  const handleImage = (e: React.ChangeEvent<HTMLInputElement>, i: number) => {
    ...
    // revoke old blob URL if this slot previously held a local preview.
    // to avoid leaking memory when swapping images
    const oldPreview = imagePreview[i]
